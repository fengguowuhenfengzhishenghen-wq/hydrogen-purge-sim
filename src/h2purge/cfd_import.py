"""Import external 3D CFD review results for Streamlit display.

This module only reads files generated by an external CFD workflow such as
Fluent or OpenFOAM. It does not synthesize, estimate, or solve CFD fields.
"""
...
def list_cfd_cases(base_dir: str | Path = "outputs/cfd3d") -> list[Path]:
    """Return external CFD case directories below *base_dir*."""
...
def load_cfd_metrics(case_dir: str | Path) -> dict[str, Any]:
    """Load metrics.json from an external CFD case directory."""
...
def load_cfd_images(case_dir: str | Path) -> dict[str, Path]:
    """Return available image paths for an external CFD case directory."""
